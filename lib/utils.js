...
function buildMissingScopeMessage (name, env, scopes) {
  ...
  scopes.map(s => buildFullyQualifiedScope(name, env, s))
  let message = 'missing scope' + scopes.length > 1 ? 's' : ''
  message += ' ' + scopes.join(' or ')
  return message
}
...
module.exports = {buildFullyQualifiedScope, buildMissingScopeMessage, checkPermissions, checkScopes}
